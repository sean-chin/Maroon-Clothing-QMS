...
Non-obvious notes:
- The app auto-loads `.env` and `.env.local` from the project root on startup (later files do not override vars already set in the shell; keys starting with `VERCEL` are skipped since those must only come from Vercel's live runtime, not a stale `vercel env pull` snapshot). Env vars include `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `ADMIN_PIN`, `PORT`, `STORE_CAPACITY`, `GUESTS_PER_MINUTE`, `ALMOST_AHEAD`, `SMTP_*`, `VAPID_*`. With no vars set, the app runs fully with defaults (admin PIN `maroon2026`, port `3000`); email and push notifications are simply disabled (in-tab browser alerts still work while a tab is open).
