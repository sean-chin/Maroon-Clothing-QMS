...
- **Guest page (`/`)**: guest enters their name (plus an optional email), gets a queue number, sees how many guests are ahead and a rough wait estimate. Page auto-refreshes every few seconds. A "Never miss your call" card lets them switch on push notifications or add an email, then go roam the mall.
- **Manager dashboard (`/admin`)**: PIN-protected. Shows waiting / called / in-store counts. When guests leave the store, the manager calls the next N guests; those guests instantly get the "it's your turn" ping on every channel they linked. Manager then marks them **Entered**, **Left store**, **No-show**, or **Re-queue**. Small PUSH / @ chips in the queue table show who gets pinged remotely.
...
- State is snapshotted atomically to `data/queue.json` on every single change (no debounce window), so a crash or restart never loses a committed guest-state change. With Supabase, every write lands directly in Postgres.
- Per-IP rate limiting on guest endpoints, backed by the store itself (in-memory for the file backend, a Postgres RPC for Supabase) so limits hold even across multiple serverless instances. Global error handlers so one bad request never takes the server down.
- A process crash (uncaught exception or unhandled rejection) flushes pending writes, sends an alert if `ALERT_WEBHOOK_URL` is set, and exits — the host restarts a clean instance instead of continuing in a possibly-corrupted state.
- The guest page only clears a guest's local session on a real "not found" from the server; a network blip while roaming the mall shows "Reconnecting" and keeps retrying instead of silently dropping their spot.
- **Reset entire queue** in the admin dashboard archives every guest first (`data/queue-archive-<timestamp>.json` locally, a `guests_archive` table on Supabase) before wiping, so a misclick during a live event is recoverable.
...
All settings are environment variables, and the app runs fine with the defaults if you set nothing. It does **not** auto-load a `.env` file, so export the variables in your shell before `npm start`. See `.env.example` for the full list (`ADMIN_PIN`, `PORT`, `STORE_CAPACITY`, `GUESTS_PER_MINUTE`, `ALMOST_AHEAD`, `ALERT_WEBHOOK_URL`, SMTP settings, VAPID keys).
...
**Vercel:** set `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` in project env (service role only — never in the browser). Run every file in `supabase/migrations/` once, in filename order, in the Supabase SQL editor (or `supabase db push`), then redeploy.
