...
- **Push notifications** (recommended, works with the tab closed): the guest taps "Turn on" once and their phone gets a real system notification, a vibration and a sound, even with the browser closed and the screen locked. On Android this works straight away. On iOS it needs the page added to the home screen first (iOS 16.4+); the guest page detects this and shows that exact instruction instead of a dead end. Inside an in-app browser (Instagram, TikTok, and the like), push and browser alerts do not work at all; the guest page detects that too and tells the guest to open the link in Safari or Chrome first. Needs `VAPID_PUBLIC_KEY` and `VAPID_PRIVATE_KEY` set (see below); without them the app falls back to a plain in-tab alert that only fires while the tab is open, which is why setting these up matters.
- **Telegram** (optional): guests tap a deep link to the bot, which links their chat via `/start <token>`. Works the same in every browser, including in-app ones, since it just opens the Telegram app. Enabled by setting `TELEGRAM_BOT_TOKEN` and `TELEGRAM_BOT_USERNAME`.
- **Email** (optional): the email field is always shown, at join and on the status page, so a guest can always leave one as a backup even if nothing else is configured yet. Sends a plain-text plus simple branded HTML message over SMTP once `SMTP_*` is set. If it is not set yet, the address is still saved; turning SMTP on later (even mid-event) starts emailing everyone who already left one, from that point on.
...
For the most reliable setup, turn on all three: push covers guests who close the tab, Telegram is the most universal fallback since it works inside in-app browsers too, and email is the safety net for anyone who skips the other two.

**If a guest reports no notifications work at all and the phone shows "Not supported here" or nothing on the Browser alerts row**, it almost always means none of `VAPID_PUBLIC_KEY`, `TELEGRAM_BOT_TOKEN`, or `SMTP_*` are set on whatever host is actually running the app (check with `GET /api/config`; `vapidPublicKey`, `botUsername`, and `emailEnabled` should all be non-empty/true once configured). The guest page always shows the email field regardless, so a guest can always leave a backup address even before that is fixed.
